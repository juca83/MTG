    ...
    <div className="surface-card p-3">
      <div className="flex items-start justify-between mb-1.5">
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          ...
          <p className="font-semibold text-xs truncate">{budget.category_name || 'Global'}</p>
        </div>
        <div className="flex gap-0.5 flex-shrink-0">
          <button onClick={() => onEdit(budget)} className="p-1 rounded-lg hover:bg-app-surface2 text-gray-400 hover:text-gray-900">
            <Pencil size={11} />
          </button>
          <button onClick={() => onDelete(budget.id)} className="p-1 rounded-lg hover:bg-red-500/20 text-gray-400 hover:text-red-400">
            <Trash2 size={11} />
          </button>
        </div>
      </div>

      <div className="h-2 bg-app-surface2 rounded-full overflow-hidden mb-1.5">
      ...
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-xs text-gray-400">{fmt(budget.amount)}</span>
      </div>
      <div className="text-right mt-0.5">
        <span className={`text-xs ${over ? 'text-red-400' : pct > 80 ? 'text-orange-400' : 'text-green-500'}`}>
          {over
            ? `+${fmt(Math.abs(budget.spent - budget.amount))} dépassé`
            : `${fmt(budget.amount - budget.spent)} restant`}
        </span>
      </div>
    </div>
  )
...
export default function Budgets() {
  ...
  const filteredBudgets = userFilter !== null
    ? budgets.filter(b => b.user_id === userFilter)
    : budgets

  const totalBudget = filteredBudgets.reduce((s, b) => s + b.amount, 0)
  const totalSpent  = filteredBudgets.reduce((s, b) => s + (b.spent || 0), 0)
    ...
    <div className="page-padding">
      ...
      {filteredBudgets.length > 0 && (
        <div className="surface-card p-4 mb-4">
          <div className="flex justify-between mb-2">
            <div>
              <p className="text-xs text-gray-400">Budgeté</p>
              <p className="text-lg font-bold">{fmt(totalBudget)}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-gray-400">Dépensé</p>
              <p className={`text-lg font-bold ${totalSpent > totalBudget ? 'text-red-400' : 'text-green-400'}`}>
                {fmt(totalSpent)}
              </p>
            </div>
          </div>
      ...
      {filteredBudgets.length === 0 ? (
        <div className="text-center text-gray-500 py-16">
          ...
          <p>{budgets.length === 0 ? 'Aucun budget ce mois-ci' : 'Aucun budget pour cette personne'}</p>
          {budgets.length === 0 && <p className="text-sm mt-1">Créez des budgets pour mieux gérer vos dépenses</p>}
        </div>
      ) : (
